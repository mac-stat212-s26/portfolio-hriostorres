...
scrape_nih_news <- function(page_num) {
  url <- str_glue("https://www.nih.gov/news-events/news-releases?page={page_num}")
  page <- read_html(url)
  
  # 1. More robust Title selector
  titles <- page |> 
    html_elements(".thumbnail-teaser__heading") |> 
    html_text2()
  
  # 2. More robust Date selector
  dates <- page |> 
    html_elements(".thumbnail-teaser__date") |> 
    html_text2()
  
  # 3. FIXED Abstract selector 
  # If .thumbnail-teaser__abstract is returning 0, 
  # try the more general teaser__description or checking the class again
  abstracts <- page |> 
    html_elements(".thumbnail-teaser__abstract") |> 
    html_text2()

  # CRITICAL FIX: The "Safety Net"
  # This prevents the 'compatible sizes' error by forcing 
  # the vectors to be the same length even if scraping fails for one.
  n <- length(titles)
  if (length(dates) != n) dates <- rep(NA, n)
  if (length(abstracts) != n) abstracts <- rep(NA, n)
  ...
  tibble(
    ...
    mutate(abstract = str_remove(abstract, fixed(date)),
...
```{r}
# Define the pages we want to scrape (0 to 4)
pages_to_scrape <- 0:4

# Use map to iterate and list_rbind to stack the results
nih_data_purrr <- pages_to_scrape |> 
  map(scrape_nih_news) |> 
  list_rbind()
...
# 2. Run the loop
for (i in 1:length(pages_to_scrape)) {
  # We use i-1 because the NIH site starts page numbering at 0
  results_list[[i]] <- scrape_nih_news(pages_to_scrape[i])
  
  # Optional: be a good "web citizen" and pause between requests
  # Sys.sleep(1) 
}

# 3. Combine
nih_data_loop <- bind_rows(results_list)
```
...
```{r exerc_2}
# Scrape the STEM resource titles

stem_url <- "https://science.education.nih.gov/"
stem_html <- read_html(stem_url)

resource_titles <- stem_html |> 
