...
mn_cities <- st_read("../ica/data/shp_loc_pop_centers/city_and_township_population_centers.shp")
mn_water <- st_read("../ica/data/shp_water_lakes_rivers/LakesAndRivers.shp")
```
...
transform_and_plot(nc, new_crs = "EPSG:5356") +
  labs(title = "Bolivia CRS")

transform_and_plot(nc, new_crs = "EPSG:3088") +
  labs(title = "Kentucky CRS")
...
names(mn_counties)[names_counties == "state_name"] <- c("namelsad", "County")
```
...
names(mn_counties)
# Let's rename the second one
names(mn_counties)[9] <- "namelsad2"
