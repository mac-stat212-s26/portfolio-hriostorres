...
```{r}
# --- The "Manual" For Loop Way (Review) ---

# 1. Create a storage container (Pre-allocation)
output <- vector("character", length = length(mtcars))

# 2. The loop: Translate steps to code
for (i in seq_along(mtcars)) {
  # Access the column with [[i]], get the class, and save it in the container
  output[i] <- class(mtcars[[i]])
}

# 3. Add names so we know which class belongs to which column
names(output) <- names(mtcars)
output


# --- The "purrr" Way (The New Way) ---
# This replaces all the lines above with just one line:
...
# To apply it to all numeric columns in diamonds:
diamonds_stats <- diamonds |> 
  select(where(is.numeric)) |> 
  map_dfr(summ_stats, .id = "variable") # .id adds the column names back!
```
...
# use pmap()
# We pass 'diamonds' as a constant argument to the 'data' parameter
model_results <- df_arg_combos |> 
  mutate(model = pmap(df_arg_combos, fit_model, data = diamonds))
