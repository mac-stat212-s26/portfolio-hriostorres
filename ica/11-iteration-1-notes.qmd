...
remove_outliers <- function(data, what_var, sd_thresh) {
  ...
  mean_val <- mean(pull(data, {{ what_var }}), na.rm = TRUE)
  sd_val   <- sd(pull(data, {{ what_var }}), na.rm = TRUE)
  
  data |>
    filter(abs({{ what_var }} - mean_val) <= sd_thresh * sd_val)}
...
fit_model <- function(data, mod_formula, remove_outliers_flag) {
  ...
  if (remove_outliers_flag) {
    data <- remove_outliers(data, carat, 3)}
...
for (i in seq_len(nrow(df_arg_combos))) {
  ...
  this_formula <- df_arg_combos$mod_formula[[i]]   # list column
  this_remove  <- df_arg_combos$remove_outliers[i] # logical vector
